...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Hero scene wrapper */
  .hero-scene .scroll-scene-content {
    perspective: 1000px;
  }
  .hero-inner {
...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    const renderFrame = () => {
      ...
      // Scroll-responsive scene adjustments — camera pulls back as hero exits
      const sp = scrollProgressRef.current;
      const targetZ = 9 + sp * 6;
      camera.position.z += (targetZ - camera.position.z) * 0.04;
      knotMat.emissiveIntensity = 0.18 * Math.max(0, 1 - sp * 1.3);
      wireMat.opacity = 0.18 + sp * 0.45;
      pts.material.opacity = Math.max(0.08, 0.45 - sp * 0.3);
      knot.scale.setScalar(1 - sp * 0.15);
      wire.scale.setScalar(1 - sp * 0.15);
  ...
  // ── SCROLL SCENE CALLBACKS ──────────────────────────────────────────────
  const heroOnProgress = useCallback((p) => {
    scrollProgressRef.current = p;
  }, []);

  const heroTimeline = useCallback((tl, content) => {
    const inner = content.querySelector(".hero-inner");
    if (!inner) return;
    // Hero content fades and lifts as you scroll through the scene
    tl.to(inner, { opacity: 0, y: "-12vh", duration: 0.55, ease: "power2.in" }, 0.3);
  }, []);

  const projectsTimeline = useCallback((tl, content) => {
    const stage = content.querySelector(".projects-stage");
    if (!stage) return;
    // Projects emerge forward from Z-depth and opacity
    tl.fromTo(
      stage,
      { y: "10vh", z: -180, opacity: 0.3, filter: "saturate(0.8)" },
      { y: 0, z: 0, opacity: 1, filter: "saturate(1)", duration: 0.65, ease: "power2.out" }
    );
  }, []);

  const aboutTimeline = useCallback((tl, content) => {
    const stage = content.querySelector(".about-stage");
    if (!stage) return;
    // About section wipes up from bottom via clip-path
    tl.fromTo(
      stage,
    ...
    <div ref={containerRef} style={{ background: "#040e09", color: "#cee8db", fontFamily: "'Manrope', system-ui, sans-serif", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 2 }}>
        ...
        <ScrollScene
          ...
          duration={200}
          onProgress={heroOnProgress}
          buildTimeline={heroTimeline}
        >
        ...
        >
          <div className="projects-stage scene-stage">
        ...
        >
          <div className="about-stage scene-stage">
            <div className="section section-glass">
